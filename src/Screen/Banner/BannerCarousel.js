...
const BannerCarousel = () => {
  const arrImage = [food1, food2, food3, food4, food5];
  ...
  const handles = e => {
    ...
    if (nativeEvent && nativeEvent.contentOffset) {
      ...
      if (currentOffset > 0) {
        ...
        console.log(
          '(currentOffset + SIZE.height / 2) / SIZE.width): ',
          (currentOffset + SIZE.width / 2) / SIZE.width,
        );
  ...
  const changeIndex = (index = 0) => {
    stepCarousel?.current?.scrollToIndex({
      index: index,
      animted: true,
      viewOffset: imageIndex,
      viewPosition: currentOffset,
    });
  };
        ...
        renderItem={({item, index}) => {
          return (
            <View style={{height: 200, width: SIZE.width}}>
              <Image
                style={{height: 200, width: SIZE.width, resizeMode: 'contain'}}
                source={item}
              />
            </View>
          );
            ...
            <View style={{marginTop: 5, height: 60}}>
              <TouchableOpacity onPress={() => changeIndex(index)}>
                <Image
                  style={
                    (imageIndex || 0) === index
                      ? [styles.unActiveBtnItemImg, {borderColor: COLOR.error}]
                      : styles.unActiveBtnItemImg
                  }
                  source={item}
                />
              </TouchableOpacity>
